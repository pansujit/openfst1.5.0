...
class GzFile {
 public:
  GzFile(const char *filename, const char *mode_)
      : gzfile_(gzopen(filename, mode_)), error_(check_handle()) { }
  ...
  explicit GzFile(const int fd, const char *mode_)
      : gzfile_(gzdopen(fd, mode_)), error_(check_handle()), close_me_(false) { }
...
class OGzFile {
 public:
  ...
  explicit OGzFile(const char *filename) : gz_(GzFile(filename, mode)) { }
  explicit OGzFile(const int fd) : gz_(GzFile(fd, mode)) { }
